...
- **AMD's stack as tested (Ryzen AI 1.7.1) exposes no multi-segment dispatch,** which is an architectural difference and not a measured speedup: in that sitting, creating an NPU hardware context cost 29.63 ms and CPU fallback 82.33 ms, so its stack ran all 66 layers every frame.
...
| YOLOv8n | Ignition split | Segment 0 only: no heads, no detections | 2.01 ms | 498.7 | — | **0.68 MB** | 8.16 MB | **174.4 MB** | 25.5% |
| YOLOv8n | Ignition split | Full network (2 segs) | 7.85 ms | 127.4 | 20.8 µs | **0.68 MB** | 8.16 MB | **174.4 MB** | 100% |
| YOLOv8s | AMD Ryzen AI 1.7.1 | Monolithic ONNX | **16.75 ms** | 59.7 | — | 30.76 MB | — | 337.4 MB | full network |
| YOLOv8s | Ignition split | Segment 0 only: no heads, no detections | 4.64 ms | 215.4 | — | **1.26 MB** | 29.50 MB | **207.1 MB** | 26.3% |
| YOLOv8s | Ignition split | Full network (3 segs) | 17.66 ms | 56.6 | 26.1 µs | **1.26 MB** | 29.50 MB | **207.1 MB** | 100% |
| YOLOv8n-pose | AMD Ryzen AI 1.7.1 | Monolithic ONNX | **11.97 ms** | 83.5 | — | 8.86 MB | — | 305.2 MB | full network |
| YOLOv8n-pose | Ignition split | Segment 0 only: no heads, no detections | 2.29 ms | 437.4 | — | **0.68 MB** | 8.18 MB | **177.6 MB** | 27.9% |
| YOLOv8n-pose | Ignition split | Full network (3 segs) | 8.21 ms | 121.8 | 17.7 µs | **0.68 MB** | 8.18 MB | **177.6 MB** | 100% |
| YOLOv8m | AMD Ryzen AI 1.7.1 | Monolithic ONNX | **26.95 ms** | 37.1 | — | 64.15 MB | — | 385.0 MB | full network |
| YOLOv8m | Ignition split | Segment 0 only: no heads, no detections | 11.24 ms | 89.0 | — | **3.15 MB** | 61.00 MB | **239.9 MB** | 25.8% |
| YOLOv8m | Ignition split | Full network (3 segs) | 43.60 ms | 22.9 | 33.1 µs | **3.15 MB** | 61.00 MB | **239.9 MB** | 100% |
| YOLOv8l | AMD Ryzen AI 1.7.1 | Monolithic ONNX | **49.67 ms** | 20.1 | — | 98.22 MB | — | 490.2 MB | full network |
| YOLOv8l | Ignition split | Segment 0 only: no heads, no detections | 19.53 ms | 51.2 | — | **5.56 MB** | 92.66 MB | **394.6 MB** | 25.0% |
| YOLOv8l | Ignition split | Full network (3 segs) | 78.22 ms | 12.8 | 45.7 µs | **5.56 MB** | 92.66 MB | **394.6 MB** | 100% |
| YOLOv8x | AMD Ryzen AI 1.7.1 | Monolithic ONNX | **117.11 ms** | 8.5 | — | 154.16 MB | — | 580.4 MB | full network |
| YOLOv8x | Ignition split | Segment 0 only: no heads, no detections | 32.95 ms | 30.3 | — | **8.67 MB** | 145.49 MB | **347.5 MB** | 26.3% |
| YOLOv8x | Ignition split | Full network (3 segs) | 125.50 ms | 8.0 | 49.9 µs | **8.67 MB** | 145.49 MB | **347.5 MB** | 100% |
