...
- **The win is placement, not speed.** AMD's stack cannot get the head onto the NPU: shown the terminal Gemm it aborts inside `vaip_core` with `runner_requests_queue.cpp:178: Failed to create runner: invalid vector subscript`, accelerating 0 nodes and falling back entirely to the CPU — the same wall detection models hit before AMD's users cut their heads off (`yolov8n_cut.onnx`). The engine runs the same head on the NPU with 0 host segments and no new core opcode: it lowers a terminal GlobalAveragePool -> Flatten -> Gemm chain to a single 1x1 convolution across the 20x20 tile (ignite-xdna `bd87558`; its `docs/DECISIONS.md` records the container matching the direct integer reference layer-for-layer, `max_diff = 0`, on that lowering). For the head alone AMD's own CPU fallback is faster — 0.666-0.707 ms against 1.378-1.386 ms — and leaner (91.3 MB against 168.8 in the first round); what the NPU path buys is that a classifier on this machine has an NPU placement at all, next to a detector, which is something AMD's stack has never offered. The full ResNet50's 30.49 ms CPU row above is the whole model on the other path, not this head's counterpart.
- **What is not wired yet:** `live_ignition.py` refuses `.ignite` classification containers (`pipelines/vision.py` raises `NotImplementedError`), so this number is the engine pipeline the SDK wraps, not a released app flag. Wiring `--task classify` to the container, and a head sitting on a real backbone's features rather than the constant zero-point vector, are open.
...
Measured on 2026-09-17 (UTC) in one sitting with Ignition in its default power mode, ignite-xdna at `1879614` (released as 0.3.1) and Ignition at `e27ef79`. Since ignite-xdna `fbd53f5`, which 0.3.1 is the first release to include, the CPU step's ONNX Runtime threads follow the power mode instead of spinning; with ignite-xdna 0.3.0 they spin in every mode. The images were `examples/assets/bus.jpg`, with 50 warm-up and 500 timed frames per run. Runs alternated AMD's stack, a container with YOLO11n's whole C2PSA attention block on the CPU, one with only its attention core there, and that one again with `--power-mode performance`. YOLO11n cannot take `--silu-sigmoid`: ignite-xdna refuses the flag for containers with CPU steps. Both containers first matched their model on every layer (84/84 and 91/91), and `xrt-smi` reported no hardware contexts before every group and at the end. AMD's runs used ignite-xdna's `tools/amd_vitisai_yolo.py`, the Vitis AI loop of Ignition's `benchmarks/benchmark_yolo_vitisai.py` with Ignition's numpy letterbox and decode. The record is ignite-xdna's `results/aie/release_033/latency_release033_phoenix_20260917T1538Z.log`.

| Run | Stack | Mean | 99th pct | Where the time goes | Objects |
|---|---|---:|---:|---|---:|
| 1 | AMD Ryzen AI Software 1.7.1 (ONNX Runtime + Vitis AI EP) | 36.77 ms | 39.26 ms | `session.run` 32.92 ms | 7 |
| 2 | Ignition, whole attention block on the CPU | 11.25 ms | 12.08 ms | NPU dispatch 8.41 ms, CPU step 1.88 ms | 6 |
| 3 | Ignition, attention core on the CPU | **10.46 ms** | 11.19 ms | NPU dispatch 8.74 ms, CPU step 0.75 ms | 6 |
| 4 | AMD Ryzen AI Software 1.7.1 (ONNX Runtime + Vitis AI EP) | 38.30 ms | 41.80 ms | `session.run` 34.35 ms | 7 |
| 5 | Ignition, whole attention block on the CPU | 11.35 ms | 12.41 ms | NPU dispatch 8.39 ms, CPU step 1.94 ms | 6 |
| 6 | Ignition, attention core on the CPU | **10.50 ms** | 11.21 ms | NPU dispatch 8.75 ms, CPU step 0.74 ms | 6 |

- **What moved to the NPU:** most of the attention block's CPU time went to its convolutions and their quantize and dequantize steps, which the NPU already runs exactly. With the block's seven convolutions and its residual adds on the NPU, the CPU step fell from 1.88–1.94 to 0.74–0.75 ms and NPU dispatch rose from 8.39–8.41 to 8.74–8.75 ms, 0.82 ms less per frame on the means. Both containers' boxes equal ONNX Runtime's CPU decode of the same input ([how](https://github.com/jdominick05/ignite-xdna/blob/main/docs/BENCHMARKS.md#yolo11ns-c2psa-convolutions-on-the-npu-only-its-attention-core-on-the-host-2026-09-15-desktop-2)).
