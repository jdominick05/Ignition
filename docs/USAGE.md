...
**Neither runs from a released wheel today, and both are slower than AMD's stack.** The `.ignite` path needs `ignite_xdna.runtime.dense_session`, which landed in the ignite-xdna source checkout on 2026-09-21 but is in no released wheel yet; from a wheel Ignition refuses with a message naming what is missing rather than the bare `ModuleNotFoundError` it raised before 0.3.4. The `.onnx` CPU path additionally needs that same source checkout, whose `npu/` transforms are not packaged. Measured on Desktop 2, BiSeNetV2 runs 2.10x and MODNet Cut 2.82x slower than AMD's stack, and both are exact against the CPU reference on all 50 pinned images where AMD's stack is exact on none. See the
[measurements](PERFORMANCE.md#segmentation-and-matting-hybrid-paths-against-amds-stack) and
[model-building rules](QUANTIZATION-GUIDE.md#explicit-segmentation-and-matting-regions).

## Limitations

- **Model coverage is explicit.** The released packages run YOLOv8n, YOLOv8s, SESR M7, YOLOv8n-pose, YOLO11n and YOLO26n on the NPU. The hybrid segmentation and matting recipes above run from the ignite-xdna source checkout but from no released wheel yet, and from a wheel Ignition refuses them with a message naming what is missing; both are slower than AMD's stack ([measurements](PERFORMANCE.md#segmentation-and-matting-hybrid-paths-against-amds-stack)). ResNet50 classification runs on the CPU in this app: the engine has a `.ignite` lowering that puts a whole classifier on the device for models it accepts, and one of fifteen XINT8 classifiers reaches a schedule at all ([measurements](PERFORMANCE.md#classification-head-against-amds-stack)); none of it is wired into `live_ignition.py` yet, and other ONNX models run on the CPU.
...
- **YOLO11n's attention core runs on the CPU.** Its matrix multiplies and softmax took 0.72–0.73 ms of a 10.27–10.29 ms frame in the default power mode with ignite-xdna 0.3.1; ignite-xdna 0.3.0 keeps that step's threads spinning in every mode. Compiled with `--silu-sigmoid` it scores 34.63 mAP@50-95 on all 5,000 COCO val2017 images against 25.82 for AMD's stack on the same harness, and its borderline boxes change with one-code differences in preprocessing: on `bus.jpg` it finds 5 objects where AMD's stack finds 7. One of the two it does not report is a quantization artifact of the HardSigmoid form that the float model does not produce either ([measurements](PERFORMANCE.md#yolo11n-with-its-attention-block)).
