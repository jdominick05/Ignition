...
**Real-time YOLO on the AMD Ryzen™ AI NPU: YOLOv8n, YOLO11n and YOLOv8n-pose run faster end to end than on AMD's own runtime, from a fraction of the install size.**
...
are harmless. Calibration images shape the result: on `bus.jpg`, a container built this way found 7 objects
where the benchmarked one finds 5.
...
ignite-compile --input ..\ignite-xdna\models\yolov8n_cut_xint8.onnx --output ..\ignite-xdna\build\yolov8n_full.ignite
python live_ignition.py
```

A window opens on webcam 0 with boxes, labels and latency; press q to quit. Add `--source picture.jpg` for an image.
